...
## 2025-05-31 - Optimizing Sorted Array Lookups in Reactive Variables

**Learning:** In `FeedSlide.svelte`, `currentSentence` was derived using `sentences.filter(s => s.start <= index).pop()`. For a reactive variable running at high frequency (e.g., 60fps scrubbing), this created unnecessary array allocations and iterated the entire array.
**Action:** Replaced the pattern with a simple reverse `for` loop. This finds the target sentence in O(N) (average case much faster) without any memory allocation, significantly reducing GC pressure during interactive scrubbing.
