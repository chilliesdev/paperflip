...
## 2025-06-01 - Optimizing Array Allocation in Reactive Blocks
**Learning:** In Svelte 5, using array methods that allocate new memory (like `.filter(...).pop()` or mapping/filtering) inside high-frequency reactive blocks (like `$derived.by` driven by playback position) causes unnecessary Garbage Collection (GC) overhead and CPU usage. Finding a single matching element using `.filter(...).pop()` iterates the entire array and creates an intermediate array just to throw it away.
**Action:** Replaced `.filter().pop()` with a direct backward `for` loop, and replaced `.filter()` mapping with imperative loops that preallocate only necessary items. Always prefer imperative loops over allocating array methods for operations that run synchronously with high-frequency events (like 60fps animations or audio progress scrubbing).
