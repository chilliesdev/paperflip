<script lang="ts">
  ...
  async function loadRecentUploads() {
    try {
      ...
      recentUploads = uploads.map((doc: any) => {
        const totalSegments = doc.totalSegments || 0;
        const currentIndex = doc.currentSegmentIndex || 0;
        const currentProgress = doc.currentSegmentProgress || 0;
        const currentSegmentLength = doc.currentSegmentLength || 1;

        let progress = 0;
        if (totalSegments > 0) {
          const granularProgress =
            currentIndex + currentProgress / currentSegmentLength;
  ...
  async function handleFileUpload(event: Event) {
    ...
    try {
      ...
      // Replaced sequential page loading with concurrent Promise.all execution.
      // This maximizes I/O throughput when reading multiple pages and drastically reduces
      // parsing time for large documents. Kept array join to prevent O(n^2) string copying.
