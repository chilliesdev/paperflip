...
- Move _all_ your current SvelteKit files (e.g., `src/`, `static/`, `vite.config.ts`, `svelte.config.js`, `package.json`, `tailwind.config.js`) into `apps/web/`.
...
- _Do not_ use `pdf.worker.min.mjs` or `pdfjs-dist` on mobile.
