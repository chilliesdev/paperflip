...
1. **Move the SvelteKit application:** Move all core SvelteKit files and folders into `apps/web/`.
   - _Folders:_ `src/`, `static/`, `tests/`.
   - _Configs:_ `package.json`, `svelte.config.js`, `vite.config.ts`, `tailwind.config.js`, `postcss.config.js`, `tsconfig.json`, `eslint.config.js`, `.prettierrc.js`, `.prettierignore`, `.npmrc`.
