<script lang="ts">
	...
	async function handlePdfParsed(event: CustomEvent<{ text: string }>) {
		...
		try {
			...
			console.log('Document and segments stored in RxDB with ID:', currentDocumentId);
		} catch (error) {
...
<div class="min-h-screen flex flex-col items-center justify-center bg-gray-100">
	...
	{:else}
		...
		<div class="p-8 bg-white rounded-lg shadow-md">
			<h2 class="text-xl font-semibold text-gray-700 mb-4">Upload a PDF to get started</h2>
			<PdfUploader on:pdf-parsed="{handlePdfParsed}" on:pdf-error="{handlePdfError}" />
		</div>
	{/if}
</div>