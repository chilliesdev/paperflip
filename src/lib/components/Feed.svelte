<script lang="ts">
    ...
    import { initializeTTS, speakText, stopTTS, pauseTTS, resumeTTS, isSpeaking } from '$lib/audio';
    ...
    function getHighlightedText(text: string, highlightWord: string): string {
        ...
        return words.map(word => {
            ...
            const cleanedHighlightWord = highlightWord.replace(/[.,!?;:]/g, '').trim().toLowerCase();
...
<div class="h-screen w-screen flex justify-center items-center relative overflow-hidden">
    {#if segments.length > 0}
        <video
        ...
        >
            {#each segments as segment, i}
                <SwiperSlide class="flex items-center justify-center p-4 relative bg-transparent">
                    <div class="text-center text-2xl text-white z-10 p-4 bg-black bg-opacity-50 rounded">
